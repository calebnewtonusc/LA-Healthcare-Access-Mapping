...
const getScorePattern = (score: number) => {
  ...
  if (score >= 60) return 'url(#pattern-good)'
  if (score >= 45) return 'url(#pattern-fair)'
  return 'url(#pattern-poor)'
}

export default function RegionalBreakdownChart() {
  return (
          ...
          <BarChart data={regionalData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
            ...
            <Bar dataKey="score" radius={[8, 8, 0, 0]}>
              {regionalData.map((entry) => (
                <Cell key={`cell-${entry.region}`} fill={getScorePattern(entry.score)} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
